""" todoy.py 
    for the moment just a simple paint program"""
...
def checkKeys(myData):
	...
	background = pygame.image.load("todoy_bkgrnd.png").convert()
	backgroundRect = background.get_rect()
    elif event.key == pygame.K_s:
        #save picture
	painting=strftime("%Y%m%d",gmtime())+".bmp"
        pygame.image.save(background, painting)
    elif event.key == pygame.K_l:
        #load picture
	painting=strftime("%Y%m%d",gmtime())+".bmp"
        background = pygame.image.load(painting)
...
def showStats(drawColor, lineWidth):
    ...
    stats = "color: %s, width: %d, date: %s" % (drawColor, lineWidth, date)
